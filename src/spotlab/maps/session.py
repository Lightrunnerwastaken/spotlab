...
# Re-Export: der Bericht gehoert zum Protokoll, nicht zur Sitzung.
Nachbearbeitung = nachbearbeitung.Nachbearbeitung
...
class RecordingSession:
    ...
    def schliesse_schleifen(self, fiducial=True, odometrie=True):
        """Siehe `maps/nachbearbeitung.py` — hier nur die Fehlerübersetzung dazu."""
        return self._versuche(
            lambda: nachbearbeitung.schliesse_schleifen(
                self._processing, fiducial, odometrie
            ),
            "Schleifen schliessen",
        )

    def optimiere_anker(self):
        return self._versuche(
            lambda: nachbearbeitung.optimiere_anker(self._processing), "Anker optimieren"
        )

    def nachbearbeiten(self, melde=None, fiducial=True, odometrie=True):
        """Schleifen schliessen und Anker optimieren. Wirft NIE — siehe
        `maps/nachbearbeitung.py`."""
        return nachbearbeitung.nachbearbeiten(self._processing, melde, fiducial, odometrie)
