...
def test_die_grenzen_kommen_aus_spotsim(monkeypatch):
    ...
    try:
        b.power_on()
        b.send_command(B.synchro_velocity_command(.8, 0, .9), end_time_secs=time.time()+60)
        assert _gesendet(b) == pytest.approx((.8, .9))
        b.send_command(B.synchro_velocity_command(1.2, 0, 1.5), end_time_secs=time.time()+60)
        assert _gesendet(b) == pytest.approx((.85, 1.0))
    finally:
        b.close()


def test_der_trab_behaelt_seine_grenzen(backend, monkeypatch):
    monkeypatch.delenv('SPOTSIM_REGLER', raising=False)
    b = backend
    b.power_on()
    b.send_command(B.synchro_velocity_command(.8, 0, .9), end_time_secs=time.time()+60)
