...
# 35 s Wanduhr allein, unter Last ein Vielfaches. Die Frist von 600 s je Richtung
# ist Sim-Zeit (`b.uhr()`); die Marke ist das Netz gegen Haengen, kein Mass
...
def test_sdk_up_and_backward_down():
    ...
    try:
        b.power_on()
        b.send_command(B.synchro_velocity_command(.02, 0, 0), end_time_secs=b.uhr()+600)
        for _ in range(28):
            b.advance(.001)
...
def test_step_mode_rejects_turning_and_high_platform():
    ...
    try:
        ...
        with pytest.raises(UnsupportedCapability, match='kein Drehen'):
            b.send_command(B.synchro_velocity_command(.01, 0, .1), end_time_secs=b.uhr()+10)
    finally:
        b.close()
